...
```bash
./fast-downward.py misc/tests/benchmarks/miconic/s1-0.pddl_helpers --search "astar(lmcut())"
```

To test the LP support use:
```bash
./fast-downward.py misc/tests/benchmarks/miconic/s1-0.pddl_helpers --search "astar(operatorcounting([lmcut_constraints()]))"
```
